...
function combineData($data) {
  $keyMap = $data->keyMap();
  $k1 = $keyMap['ADJUSTMENT'];
  $k2 = $keyMap['UNIT_MEASURE'];
  // combine data in above dimensions to single key
  $data->updateValues(function($v, $idx) use($k1, $k2, $keyMap, $data) {
    if ($idx['ADJUSTMENT'] == $k1[0]
        && $idx['UNIT_MEASURE'] == $k2[0]) {
      $v = NULL;
      foreach ($keyMap['ADJUSTMENT'] as $p1) {
        foreach ($keyMap['UNIT_MEASURE'] as $p2) {
          $idx['ADJUSTMENT'] = $p1;
          $idx['UNIT_MEASURE'] = $p2;
          $r = $data->atIndex($idx);
          if ($r !== NULL) {
            if ($v !== NULL) {
              throw new Exception("cannot combine multiple non-null values for ".$idx['REF_AREA']);
            }
            $v = $r;
          }
        }
...
updateProjects(array('macro-economic-indicators'),
  array("MNA.Q.Y..W2.S1.S1.B.B1GQ._Z._Z._Z.XDC.LR.GY" => "https://sdw-wsrest.ecb.europa.eu/service/data/MNA/Q.Y.AT+BE+CY+DE+EE+ES+FI+FR+GR+HR+IE+IT+LT+LU+LV+MT+NL+PT+SI+SK+I8.W2.S1.S1.B.B1GQ._Z._Z._Z.EUR+XDC.LR.GY?startPeriod=2000",
        "MNA.Q.Y..W2.S1.S1.B.B1GQ._Z._Z._Z.EUR.LR.N" => "https://sdw-wsrest.ecb.europa.eu/service/data/MNA/Q.Y.AT+BE+CY+DE+EE+ES+FI+FR+GR+HR+IE+IT+LT+LU+LV+MT+NL+PT+SI+SK+I8.W2.S1.S1.B.B1GQ._Z._Z._Z.EUR.LR.N?startPeriod=2000"),
  $upto,
  $force,
  $ConfigEdit);
